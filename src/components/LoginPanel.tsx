...
// É o conteúdo do login do portal (entrar · recuperar senha · criar conta em
// 3 passos · sucesso), sem a moldura. Consumido por:
//   • LoginModal   → dentro do modal, com a foto de Florianópolis ao lado;
//   • /whatsapp    → dentro do card, sem foto, com o título do fluxo e o botão
//                    "Autenticar" no lugar de "Entrar".
// Campos, rótulos, ordem, máscaras e validações do cadastro são os mesmos nos
// dois lugares porque são literalmente o mesmo código.

export type PanelView = 'login' | 'recovery' | 'cadastro' | 'sucesso';
type TipoPessoa = 'fisica' | 'juridica';
interface EmailAdicional { id: number; email: string }

const STEPS_CADASTRO = ['Acesso', 'Perfil', 'Endereço'];
...
export default function LoginPanel({
  ...
  viewInicial?: PanelView;
  formAberto?: boolean;                 // já abre o formulário de credenciais
  titulo?: React.ReactNode;             // padrão: "Boas vindas ao FloripaOn"
  ...
  const req = <span style={{ color: 'var(--error-required)' }}>*</span>;
          ...
          {(['fisica','juridica'] as TipoPessoa[]).map(tp => (
            <label key={tp} style={{ display: 'flex', alignItems: 'center', gap: 7, cursor: 'pointer', fontSize: 14, fontWeight: 500, color: 'var(--neutral-dark-pure)' }}>
              <input type="radio" name="tipoCad" value={tp} checked={tipo === tp} onChange={() => setTipo(tp)}
                style={{ accentColor: 'var(--primary-pure)', width: 15, height: 15, cursor: 'pointer' }} />
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        {tipo === 'fisica' ? (<>
          ...
          <div style={{ ...col, maxWidth: isMobile ? '100%' : '50%' }}>
            <span style={lbl}>Telefone {req}</span>
            <input style={inp} placeholder="(48) 99999-9999" value={telefone} onChange={e => setTelefone(fmtTel(e.target.value))} />
          </div>
        </>)}
        ...
        <div style={{ border: '1px solid var(--neutral-light-down)', borderRadius: 9, overflow: 'hidden' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', background: 'var(--bg-subtle)', borderBottom: emailsAdic.length > 0 ? '1px solid var(--neutral-light-medium)' : 'none', flexWrap: 'wrap', gap: 8 }}>
            <span style={{ fontWeight: 600, fontSize: 13, color: 'var(--neutral-dark-pure)' }}>E-mail Adicional</span>
            <div style={{ display: 'flex', gap: 7 }}>
              <input
                style={{ height: 32, border: '1px solid var(--neutral-light-down)', borderRadius: 6, padding: '0 9px', fontSize: 12, outline: 'none', width: isMobile ? 130 : 170 }}
              ...
              />
              <button onClick={addEmailAdic} style={{ height: 32, padding: '0 12px', borderRadius: 6, background: 'var(--primary-pure)', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 5, fontSize: 12, fontWeight: 600, color: 'white' }}>
          ...
          {emailsAdic.length === 0
            ...
            : <>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 36px', padding: '7px 14px', background: 'var(--bg-subtle)', borderBottom: '1px solid var(--neutral-light-medium)' }}>
              ...
              {emailsAdic.map((ea, i) => (
                <div key={ea.id} style={{ display: 'grid', gridTemplateColumns: '1fr 36px', padding: '9px 14px', borderBottom: i < emailsAdic.length - 1 ? '1px solid var(--neutral-light-medium)' : 'none', alignItems: 'center' }}>
                  <span style={{ fontSize: 13, color: 'var(--neutral-dark-pure)', wordBreak: 'break-all' }}>{ea.email}</span>
                  <button onClick={() => setEmailsAdic(prev => prev.filter(e => e.id !== ea.id))}
                    style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--error-color)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 4, borderRadius: 4 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '148px 1fr 1fr', gap: 12 }}>
          <div style={col}>
            ...
            <span style={{ fontSize: 11, color: 'var(--primary-pure)', fontWeight: 600, cursor: 'pointer', textDecoration: 'underline', marginTop: 1 }}>Consultar CEP</span>
          </div>
          <div style={col}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 9, padding: '12px 14px', background: 'var(--bg-subtle)', borderRadius: 9, border: '1px solid var(--neutral-light-down)', marginTop: 4 }}>
          <input type="checkbox" id="aceiteT" checked={aceitouTermos} onChange={e => setAceitouTermos(e.target.checked)}
            style={{ accentColor: 'var(--primary-pure)', width: 15, height: 15, marginTop: 2, cursor: 'pointer', flexShrink: 0 }} />
          <label htmlFor="aceiteT" style={{ fontSize: 12, color: 'var(--neutral-dark-pure)', lineHeight: 1.6, cursor: 'pointer' }}>
            Li e aceito os{' '}
            <span onClick={e => { e.preventDefault(); setShowTermos(true); }} style={{ color: 'var(--primary-pure)', fontWeight: 600, textDecoration: 'underline', cursor: 'pointer' }}>Termos de uso</span>
            {' '}e{' '}
            <span onClick={e => { e.preventDefault(); setShowTermos(true); }} style={{ color: 'var(--primary-pure)', fontWeight: 600, textDecoration: 'underline', cursor: 'pointer' }}>Política de privacidade</span>
          </label>
        </div>
      </div>
    );
  ...
  function renderNavButtons() {
    return (
      <div style={{ display: 'flex', gap: 10, marginTop: 20 }}>
        ...
        {step < 3 ? (
          <button onClick={() => setStep(s => s + 1)}
            style={{ flex: 1, height: 44, border: 'none', borderRadius: 9, background: 'var(--primary-pure)', cursor: 'pointer', fontWeight: 600, fontSize: 13, color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7, transition: 'background 0.13s' }}
          ...
          <button
            onClick={() => { if (!aceitouTermos) return; if (onCadastroCriado) onCadastroCriado(); else setView('sucesso'); }}
            disabled={!aceitouTermos}
            style={{ flex: 1, height: 44, border: 'none', borderRadius: 9, background: aceitouTermos ? 'var(--primary-pure)' : '#b0bac7', cursor: aceitouTermos ? 'pointer' : 'not-allowed', fontWeight: 700, fontSize: 13, color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7, transition: 'background 0.13s' }}
            onMouseEnter={e => { if (aceitouTermos) (e.currentTarget as HTMLButtonElement).style.background = 'var(--primary-pure-hover)'; }}
            onMouseLeave={e => { if (aceitouTermos) (e.currentTarget as HTMLButtonElement).style.background = 'var(--primary-pure)'; }}>
  ...
  const modoCadastro = view === 'cadastro' || view === 'sucesso';
  ...
  return (
    <>
      ...
      <div style={{
        ...
        justifyContent: marcaCentralizada && !mostrarFechar ? 'center' : 'space-between',
        marginBottom: isMobile ? 8 : 0,
      }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          {view === 'cadastro' && !marcaCentralizada && (
            <button onClick={() => { setView('login'); setStep(1); }}
              style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--primary-pure)', padding: '0 4px 0 0', display: 'flex', alignItems: 'center' }}>
              <FAIcon icon="fa-regular fa-arrow-left" style={{ fontSize: 14 }} />
            </button>
          )}
          <span style={{ fontWeight: 700, fontSize: marcaCentralizada ? (isMobile ? 22 : 26) : 28, color: 'var(--primary-pure)', letterSpacing: '-0.5px', lineHeight: 1 }}>
            FloripaOn
          </span>
        </div>
        {mostrarFechar && (isMobile || modoCadastro) && onFechar && (
      ...
      {view === 'cadastro' && (
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', marginTop: 20 }}>
          ...
          <StepIndicator step={step} steps={STEPS_CADASTRO} />
          {step === 1 && renderStep1()}
          {step === 2 && renderStep2()}
          {step === 3 && renderStep3()}
          {renderNavButtons()}
          <p style={{ textAlign: 'center', fontSize: 12, color: 'var(--neutral-dark-medium)', margin: '16px 0 0' }}>
            Já possui uma conta?{' '}
            <span onClick={() => { setView('login'); setStep(1); }} style={{ color: 'var(--primary-pure)', fontWeight: 600, textDecoration: 'underline', cursor: 'pointer' }}>Entrar</span>
          </p>
        </div>
      )}

      {/* ── SUCESSO (cadastro no portal) ── */}
      {view === 'sucesso' && (
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 20, textAlign: 'center', padding: '24px 0' }}>
          <div style={{ width: 72, height: 72, borderRadius: '50%', background: 'var(--success-bg)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <FAIcon icon="fa-solid fa-circle-check" style={{ fontSize: 40, color: 'var(--success-color)' }} />
          </div>
          ...
          <button
            onClick={() => { setView('login'); setStep(1); abrirForm(); }}
            style={{ height: 46, padding: '0 32px', borderRadius: 9, background: 'var(--primary-pure)', border: 'none', cursor: 'pointer', fontWeight: 700, fontSize: 14, color: 'white', transition: 'background 0.13s' }}
            onMouseEnter={e => (e.currentTarget as HTMLButtonElement).style.background = 'var(--primary-pure-hover)'}
            onMouseLeave={e => (e.currentTarget as HTMLButtonElement).style.background = 'var(--primary-pure)'}>
            Fazer login
          </button>
      ...
      {view === 'recovery' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16, flex: 1, marginTop: marcaCentralizada ? 14 : 32 }}>
          ...
          <button onClick={onGovBr}
            style={{ width: '100%', height: 52, border: 'none', borderRadius: 10, background: 'var(--govbr-color)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, fontWeight: 700, fontSize: 15, color: 'white', boxShadow: '0px 2px 8px rgba(19,81,180,0.30)', transition: 'background 0.12s' }}
